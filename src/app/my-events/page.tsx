...
export default function MyEventsPage() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      setError('Ocorreu um erro ao carregar seus eventos. Tente novamente mais tarde.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">Gerencie eventos que você organiza ou tem interesse</p>
        </div>
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
          ...
          >
            Organizados
            {myEvents.length > 0 && (
              <span className={`ml-2 py-0.5 px-2 rounded-full text-xs ${activeTab === 'organized' ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-600'}`}>
                {myEvents.length}
              </span>
            )}
          </button>
          ...
          >
            ...
            {interestedEvents.length > 0 && (
              <span className={`ml-2 py-0.5 px-2 rounded-full text-xs ${activeTab === 'interested' ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-600'}`}>
        ...
        <div>
          {myEvents.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">Nenhum evento organizado</h3>
        ...
        <div>
          {interestedEvents.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">Você ainda não mostrou interesse em eventos</h3>
              <p className="mt-1 text-sm text-gray-500">Encontre eventos e marque seu interesse.</p>
              <div className="mt-6">
                <Link
                  href="/events"
                  className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary-600 hover:bg-primary-700"
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {interestedEvents.map((userEvent) => (
                <div key={userEvent.id} className="bg-white overflow-hidden shadow rounded-lg">
                  <div className="relative h-48 w-full">
                    <Image
                      src={userEvent.event.imageUrl || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87'}
                    ...
                    <div className="absolute top-2 right-2">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        ...
                        {userEvent.status === 'GOING' ? 'Vou participar' : 'Tenho interesse'}
                      </span>
                    </div>
                  </div>
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="text-lg font-medium text-gray-900">{userEvent.event.title}</h3>
                      <span className="bg-primary-100 text-primary-700 px-2 py-1 text-xs rounded-full">
                        {format(new Date(userEvent.event.date), 'dd MMM', { locale: ptBR })}
                      </span>
                    </div>
                    <div className="text-gray-500 text-sm mb-4">
                      <div className="flex items-center">
                        ...
                        {userEvent.event.online ? 'Online' : userEvent.event.location}
                      </div>
                    </div>
                    <div className="flex space-x-2">
                      <Link
                      ...
                      >
                        {userEvent.status === 'GOING' ? 'Marcar interesse' : 'Confirmar presença'}
                      </button>
                      <span className="text-gray-300">|</span>
                      <button
